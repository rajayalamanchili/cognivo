...
| Embedding model | Voyage AI's `voyage-3`, called via LiteLLM's `embedding()` function (`TUTOR_EMBEDDING_MODEL`, `VOYAGE_API_KEY`) | Decided at Milestone 9 `/speckit-plan` time (`specs/012-tutor-agent/research.md` §1). Anthropic's default chat-completion provider has no embeddings API of its own; Voyage is Anthropic's own recommended embeddings partner and integrates through the LiteLLM dependency already used everywhere else, avoiding a second embeddings-only provider account for a single-purpose need. Unaffected by the budget-driven `LLM_PROVIDER` chat-completion switch (Backend/API table, below) -- embeddings are a separate axis, and deliberately **not** given an equivalent provider switch (considered 2026-09-05): `content_passage_embedding.py`/`grading_response_cache.py`'s `EMBEDDING_DIMENSION = 1024` is a fixed `pgvector` column width sized to `voyage-3`'s output, so (a) OpenAI's and Gemini's default embedding models don't fit that width at all, and (b) even a same-width provider (Cohere's `embed-v3` also outputs 1024 dims) produces vectors that aren't comparable to Voyage's in the same similarity search -- a live provider switch would silently corrupt retrieval/grading-cache results for every row embedded under the old provider, not just error. Revisit only with a real migration + full re-embed plan, not as a copy of the chat-completion switch. |
...
| Concern | Choice | Rationale |
|---|---|---|
| Classifier approach | Voyage AI `voyage-3` embedding (already locked for the Tutor Agent, Milestone 9) of a learner's incorrect free-text answer, fed into a small per-subject `scikit-learn` (new dependency) classifier trained offline on this project's own accumulated grading data | `specs/013-misconception-classifier/research.md` §1. Neither Anthropic nor the OpenAI models reachable via the budget-driven `LLM_PROVIDER` switch (Backend/API table, below) expose a fine-tuning endpoint this project uses; adopting a third LLM provider purely to fine-tune one narrow classifier would still be disproportionate. Reusing the already-locked embedding call and adding one boring, standard ML library is the genuinely "lightweight, trained classifier" roadmap.md's Milestone 11 scope asks for. |
...
| Confidence threshold | `0.6` default (`CONFIDENCE_THRESHOLD` in `classify.py`), overridable per job-run via the `MISCONCEPTION_CONFIDENCE_THRESHOLD` env var | `research.md` §5. Read at job-run time, never persisted per-classification, so it's tunable without a schema change -- this is how the low-confidence, correctly-withheld scenario was validated end to end. |
| Cron batch size cap | `20` pairs/run default (`MAX_PAIRS_PER_RUN` in `classify.py`), overridable via `MISCONCEPTION_MAX_PAIRS_PER_RUN`; processed oldest-qualifying-evidence-first, excess deferred to a later run via the existing watermark | `research.md` §3 (Principle IX). Bounds the cron route's worst-case duration inside the backend function's shared `maxDuration: 30`, since it has no override of its own. |
