...
(a) A pre-existing real (`is_demo=False`) `LearnerProfile` row and
its `AssessmentEvent` row are unchanged before vs. after a full harness
run. (b) No `eval-harness-*` rows remain in `learner_profiles`,
...
def test_real_data_untouched_and_synthetic_rows_cleaned_up(
    ...
    real_learner = LearnerProfile(display_name="Real Learner", is_demo=False)
    ...
    real_learner_count_before = (
        db_session.query(LearnerProfile).filter(LearnerProfile.is_demo.is_(False)).count()
    )
    ...
    leftover_learners = (
        ...
        .filter(LearnerProfile.display_name.like(f"{EVAL_HARNESS_LEARNER_PREFIX}%"))
        .count()
    )
