...
- [ ] T008 Generalize `compute_quiz_summary` in `backend/src/services/quiz/session.py` to accept either a `QuizSession` or `PracticeSession` and include `time_limit_seconds`/`elapsed_seconds`/`end_reason` in its return shape, read from the session's `timed_session_ended` event if one exists, else all three `null` (data-model.md, contracts/api.md, FR-007) (same file as T006/T007, sequential)
