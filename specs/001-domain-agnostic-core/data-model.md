...
| `band` | enum | Derived, not independently stored authority: `struggling` (< 0.4), `developing` (0.4-0.7), `mastered` (>= 0.7). Computed from `p_mastery` at read time to avoid drift between the two. |
...
| `answer_key` | JSON | For `multiple_choice`: the correct option's index/id. For `numeric`: the correct value (+ optional tolerance). Generated together with `stem`/`options`, never after the fact (FR-007, Constitution Principle II). |
...
| `flagged_by` | FK -> DemoLearnerProfile, nullable | Set when a learner/instructor flags the question (FR-011). |
