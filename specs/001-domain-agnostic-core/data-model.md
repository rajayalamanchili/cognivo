...
broken by the topic's position in the content artifact's authored
topic order (`Topic` table's declaration order) -- fully deterministic
given the same `MasteryState` rows and content artifact. If zero
