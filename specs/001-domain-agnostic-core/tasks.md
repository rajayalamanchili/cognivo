...
- [X] T060 Provision the Neon project's `production` and `staging` branches and wire the Vercel↔Neon integration for ephemeral per-preview/per-developer branches, per tech-stack.md's Data layer "Environment provisioning" row; set each persistent environment's `DATABASE_URL` in the corresponding Vercel project settings -- NOTE: Neon `production`/`staging` branches exist and are both migrated to head (`c36281670dbc` → `21a9819b3e22`, run manually via `alembic upgrade head`, 2026-08-16). Vercel project created; `DATABASE_URL` plus `ANTHROPIC_API_KEY`/`LANGFUSE_*`/`ASSESSMENT_GEN_MODEL` set directly in the Vercel dashboard by the user (not via CLI -- no `vercel` CLI/credentials available in the implementation sandbox). NOT yet confirmed: whether the Neon-Vercel marketplace integration is wired for auto-provisioned ephemeral per-preview branches, vs. the two persistent branches being set up by hand.
- [X] T061 Configure Vercel deployment (combined FastAPI + Next.js Services) in `vercel.json` / project settings per tech-stack.md, including running `alembic upgrade head` against the `production` and `staging` branches as an explicit deploy step per tech-stack.md's "Migrations per environment" row (depends on T060) -- NOTE: repo-side config done (`vercel.json`, `backend/api/index.py` ASGI entrypoint, `backend/requirements.txt` generated via `uv export` for Vercel's Python builder, `backend/scripts/deploy_migrate.sh` for the explicit per-environment migration step). Uses the long-documented `builds`/`routes` schema, not yet verified against a real deploy. `alembic upgrade head` has been run manually against both branches (see T060) but `deploy_migrate.sh` is not yet wired as an actual Vercel Deploy Hook -- future migrations still need a manual run or that wiring.
- [X] T062 Playwright deployment smoke test covering the placement-through-first-question flow against the live Vercel URL (SC-007) in `frontend/tests/e2e/smoke.spec.ts` (depends on T061) -- NOTE: spec written and registers correctly (`npx playwright test --list`), drives the demo badge + placement + first-practice-question flow generically (question content/type is LLM-generated per run). NOT run against a live URL yet -- Neon/Vercel are provisioned (T060) but no deploy has been triggered/confirmed and no live URL has been given to this session. Run via `PLAYWRIGHT_BASE_URL=<deployed-url> npm run test:e2e` once a deploy exists.
- [ ] T063 Run quickstart.md validation end to end against the deployed environment and record results (depends on all prior tasks) -- NOTE: NOT done. T060/T061's provisioning/config is in place, but no live deployment has been triggered/confirmed yet -- needs a real Vercel deploy URL before this can run.
