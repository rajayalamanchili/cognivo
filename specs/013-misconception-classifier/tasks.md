...
- [X] T020 [US1] Add `classify_misconceptions_route` to `backend/src/api/routes/cron.py`, mirroring the existing `reset-demo-data` route's auth pattern exactly: scans learner/topic pairs with newly-qualifying evidence, calls `classify.py` (T017) for each, never raises out of the batch on a single pair's failure, and returns `{"status": "ok", "classified_count": N}` counting how many pairs were actually classified this run (contracts/api.md) (depends on T017; satisfies T016). **Done**: extracted `_require_cron_secret()` as a small shared helper between both cron routes (minor refactor, not a new abstraction layer). `run_classification_batch()` (in `classify.py`) does the pair enumeration, scoped to pairs with newly-qualifying evidence since the last run (research.md §3, contracts/api.md): the watermark is each pair's existing `misconception_classified` event's own `created_at` (no new column/table), so a pair with no free-text answer newer than its last classification is skipped rather than re-embedded and re-written every run. **Correction post-review (2026-09-01)**: the initial implementation had no cursor at all -- every qualifying pair was re-evaluated on every scheduled run, producing a duplicate `misconception_classified` event daily even with zero new activity and re-embedding each pair's full history regardless of what was new. Fixed to match this doc's and contracts/api.md's original "since the last run" wording, which the code had silently drifted from. **Second correction, same day**: the first watermark fix advanced on *any* free-text `ANSWER_SUBMITTED` event, but `classify_learner_topic` only ever treats *incorrect* answers as evidence (`_qualifying_events`'s `payload.get("correct") is False` filter) -- a subsequent correct answer on an already-classified pair advanced the watermark anyway and re-triggered a duplicate classification of unchanged evidence. `_last_answer_at_by_pair` now applies the same incorrect-only filter as `_qualifying_events` before computing the watermark. **Third correction, same day**: flagged as a Principle IX risk -- the route shares the backend function's single `maxDuration: 30` with every other endpoint, and `run_classification_batch()` had no cap on how many pairs (each needing a Voyage call per qualifying answer) it would process in one invocation, an unbounded-worst-case-duration risk as volume grows. Added `MAX_PAIRS_PER_RUN` (env var `MISCONCEPTION_MAX_PAIRS_PER_RUN`, default 20, research.md §3): each run processes at most that many pairs, oldest-evidence-first, deferring any excess to a later run via the existing watermark. Also cached `_load_classifier`'s result per subject across one run's pairs (was re-deserializing `classifier.joblib` once per pair). Per-pair exception handling is explicitly deferred to T027 (US2), not added here.
...
- [X] T031 [US3] Implement `backend/scripts/check_misconception_classifier_eval.py`, mirroring `check_grading_agent_eval.py`'s structure: runs both the trained classifier (T017) and the baseline (T030) against `misconception_ground_truth.jsonl` (T010), computes and prints both accuracies, and exits non-zero **only** on a crash or malformed fixture -- never merely because the classifier scores below the baseline (research.md §7, FR-007) (depends on T017, T030, T010; satisfies T029). **Done 2026-08-31**, run for real (Claude Haiku baseline calls + real Voyage embeddings, no mocks): classifier 79%, baseline 93%, n=14 -- the classifier does not beat the baseline, printed as a `NOTE`, exit code confirmed `0`. **Correction post-review (2026-09-01)**: that 79% was train/validation leakage -- it scored the shipped `classifier.joblib` against the exact same 7-rows-per-subject it was trained on, not a genuine generalization estimate, so it was never actually comparable to the baseline's honest zero-shot 93%. With only 7 rows/subject there's no volume for a real held-out split, so the script now measures classifier accuracy via leave-one-out cross-validation instead (each row scored by a classifier fit on every *other* row for that subject, via the new shared `fit_classifier()` in `train_misconception_classifier.py`) -- the shipped artifact itself is untouched. Re-run for real: **classifier 29%, baseline 93%, n=14** -- close to chance across 3 classes per subject, an honest and considerably starker reflection of the 7-example training set than the leaked 79% ever showed.
