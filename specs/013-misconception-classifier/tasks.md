...
- [ ] T004 Add `AssessmentEventType.MISCONCEPTION_CLASSIFIED = "misconception_classified"` to `backend/src/models/enums.py` (data-model.md)
...
- [ ] T013 [P] [US1] Integration test: running the classification job for a learner/topic with sufficient matching evidence writes exactly one `misconception_classified` `AssessmentEvent` with a non-empty `cited_event_ids` list (data-model.md) in `backend/tests/integration/test_misconception_classification_job.py`
...
- [ ] T016 [P] [US1] Integration test: `GET /api/cron/classify-misconceptions` requires `Authorization: Bearer $CRON_SECRET` (`hmac.compare_digest`), returns `503` if unconfigured and `401` on mismatch -- mirroring the existing `reset-demo-data` cron route's own auth tests (contracts/api.md) in `backend/tests/integration/test_cron_classify_misconceptions_auth.py`
...
- [ ] T024 [P] [US1] Render the `misconception` field (description + evidence) on the frontend's existing weak-area display component when present, unchanged when `null` (depends on T023)
