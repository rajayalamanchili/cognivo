...
- [~] T046 Run `quickstart.md`'s 13 validation scenarios end to end against a live environment -- real Claude generation and grading calls, a real A2A network call to the deployed Grading Agent -- and record results (depends on all prior tasks). **Not runnable from this sandbox** (no live Postgres, no `ANTHROPIC_API_KEY`, no deployed Grading Agent -- same constraints noted throughout T005/T013-T041). T044's live environment now exists (both envs, 2026-08-21) -- this task is unblocked but the 13-scenario run itself hasn't been performed yet. What *was* verified locally in this sandbox as a substitute, scenario-by-scenario: backend unit+integration suite collects and passes (115 passed/97 skipped, T042), frontend unit suite passes (28/28, T042), `check_no_subject_conditionals.py` passes (T043), `check_grading_agent_test_independence.py` passes (T038), `grading-agent`'s own pytest suite passes (7/7), and `check_grading_agent_eval.py`'s full plumbing runs end-to-end short of the live LLM call (T040). Scenarios 9 (DB-backed rate limit) and 11 (live A2A deployment boundary) in particular require infrastructure (a real Postgres instance, a deployed Grading Agent) this sandbox has neither of -- both need the live environment T044 provisions.
