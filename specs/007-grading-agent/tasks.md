...
- [~] T044 Deploy `grading-agent/` to its own Vercel project (both `staging` and `main` environments per `tech-stack.md`'s Branching table) and set `GRADING_AGENT_URL` on the backend's Vercel project for both environments (depends on T003, T010). **Blocked on the same external action T003 flagged**: provisioning a new Vercel project (dashboard/CLI, org access) is a user action this sandbox cannot perform. Also needs `ANTHROPIC_API_KEY` (referenced by T041's new CI workflow) provisioned as a repo secret if not already present.
