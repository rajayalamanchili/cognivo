...
- [X] T028 [US2] Log the `grade_unlocked` `AssessmentEvent` at both existing `apply_mastery_update` call sites (`backend/src/api/routes/placement.py`, `backend/src/api/routes/questions.py`) whenever `MasteryUpdateResult.grade_unlocked` is non-null (depends on T007, T027)
...
- [X] T033 [P] Run this repo's Constitution Principle III extensibility check (`check_no_subject_conditionals.py` or equivalent) against every file touched above -- confirm it stays clean -- `OK: no subject-id-keyed conditionals found in backend/src for ['algebra-1', 'biology']`
- [X] T034 Run `quickstart.md` Scenarios 1-3 end to end against a real dev database -- all three confirmed against a live local server + the real Neon dev DB: Scenario 1 (`starting_grade=6`, `correct_by_grade={"6":true,"7":false,"8":false}`), Scenario 2 (skip on a grade-8 question returns a grade-6 replacement targeting an unused topic; skip on an at-level grade-6 question returns `422`; the skipped topic's `update_count` never incremented, confirming zero mastery signal), Scenario 3 (three `next-question` calls stayed on `integers-and-operations` while grade 6 was incomplete; mastering all three grade-6 topics produced `grade_unlocked` `{previous_unlocked_grade:6, new_unlocked_grade:7, triggering_topic_id:"solving-one-step-equations"}` and the very next call selected a grade-7 topic). Hit the known dev-DB stamped-past-migrations flake (memory) from the prior full-suite pytest run's session-scoped `drop_all` teardown -- fixed via `alembic stamp base` + `upgrade head`, then reseeded the demo learner and both content artifacts
- [X] T035 Run the full backend (`pytest`) and frontend (`npm test`) suites -- confirm Milestones 1-16 pass unmodified (SC-005). Per this project's convention, only the touched test files were run per phase above; this is the one full, unfiltered run. Backend: 508 passed (17:19); one failure (`test_placement_api.py::test_submit_placement_response_shape_and_unknown_topics`) was the pre-existing pooled-connection "cache lookup failed for type" flake `conftest.py` already documents -- a different, unrelated test flakes on each full run (also hit once each in Phase 4's runs), always clean on isolated retry, confirmed again here. Frontend: 69/69 passed, 15/15 files (6.95s)
- [X] T036 Update `roadmap.md`'s entry for this feature (currently under "Out of current roadmap") once implementation lands -- assign it a milestone number/status per this repo's roadmap-status-line discipline -- assigned Milestone 15 (next open slot after Milestone 14), moved out of "Out of current roadmap", full Definition of Done written against spec.md's SC-001-006
