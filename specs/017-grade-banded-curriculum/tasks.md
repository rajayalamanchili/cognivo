...
- [ ] T001 [P] Add `GradeBand` model (`subject_id`, `grade`, composite PK, `CHECK (grade BETWEEN 1 AND 12)`) in `backend/src/models/grade_band.py` (data-model.md)
- [ ] T002 [P] Add `LearnerGradeProgress` model (`learner_id`, `subject_id`, `unlocked_grade`, `updated_at`, composite PK) in `backend/src/models/learner_grade_progress.py` (data-model.md)
- [ ] T003 [P] Add nullable `grade: int | None` column to `Topic` in `backend/src/models/topic.py`, `ForeignKeyConstraint(["subject_id", "grade"], ["grade_bands.subject_id", "grade_bands.grade"])` (data-model.md)
- [ ] T004 [P] Add nullable `grade: int | None` and `placement_session_id: uuid.UUID | None` columns to `GeneratedQuestion` in `backend/src/models/generated_question.py` (data-model.md)
- [ ] T005 [P] Add `GRADE_ASSIGNED`, `GRADE_UNLOCKED`, `PLACEMENT_QUESTION_SKIPPED` members to `AssessmentEventType` in `backend/src/models/enums.py` (data-model.md)
- [ ] T006 Alembic migration in `backend/alembic/versions/<hash>_grade_banded_curriculum_schema.py`: create `grade_bands`; add `topics.grade` + FK; add `generated_questions.grade` + `generated_questions.placement_session_id`; create `learner_grade_progress` -- additive-only, no backfill (data-model.md's Migration section) (depends on T001, T002, T003, T004)
- [ ] T007 Alembic migration in `backend/alembic/versions/<hash>_grade_event_types.py`: `ALTER TYPE` to add the three new `AssessmentEventType` values, matching the existing enum-extension migration precedent (depends on T005)
- [ ] T008 Update `backend/src/services/content_artifact/validator.py`: parse and validate an optional top-level `grade_bands` list (`grade` int 1-12, unique per subject) and an optional per-topic `grade` field; enforce research.md Decision 1's all-or-nothing rule (if `grade_bands` is non-empty, every topic MUST declare a `grade` that references a declared band; if `grade_bands` is empty/absent, no topic may declare a `grade`)
- [ ] T009 Update `backend/src/services/content_artifact/loader.py`: persist `GradeBand` rows (delete-and-recreate per subject, same as `PrerequisiteEdge` since nothing references them) and `Topic.grade` in `persist_content_artifact` (depends on T001, T003, T008)
- [ ] T010 [P] Unit tests for the validator's grade rules in `backend/tests/unit/test_content_artifact_validator.py`: all-or-nothing rejection (some topics graded, some not), undeclared-grade rejection, out-of-range grade rejection, and a fully valid graded artifact accepted (depends on T008)
- [ ] T011 Retrofit `backend/content/algebra-1/subject.yaml` with `grade_bands` (e.g. grades 6-8) and a `grade` on every topic, contiguous with the existing prerequisite graph -- otherwise this feature has no subject to demo against (plan.md Scale/Scope) (depends on T009)
- [ ] T012 Reload `algebra-1`'s content artifact against the dev DB and confirm `biology`'s artifact still loads unchanged (SC-005 smoke check) (depends on T011)
...
- [ ] T022 [P] [US1] Vitest test for the grade label display, alongside the existing placement flow tests in `frontend/` (depends on T021)
...
- [ ] T023 [P] [US2] Unit tests for the grade-aware `rank_eligible_topics` filter in `backend/tests/unit/test_sequencing.py`: a topic above `unlocked_grade` is never eligible regardless of band/prerequisites; an ungraded topic (`grade IS NULL`) is unaffected; a topic at or below `unlocked_grade` behaves exactly as today
- [ ] T024 [P] [US2] Unit tests for the unlock check in `backend/tests/unit/test_mastery_tool.py`: completing every topic in the learner's current grade returns `grade_unlocked` equal to the next declared grade; an incomplete grade returns `grade_unlocked=None`; completing the subject's highest declared grade returns `None` with no error (no next grade to unlock); an ungraded topic never triggers the check
- [ ] T025 [P] [US2] Integration test in `backend/tests/integration/test_grade_progression.py`: place a learner, drive every topic in their starting grade to mastered except one, confirm `next-question` never selects a higher grade; master the last topic, confirm a `grade_unlocked` `AssessmentEvent` is recorded and a higher-grade question becomes selectable (SC-002, SC-003, SC-006)

### Implementation for User Story 2

- [ ] T026 [US2] In `backend/src/agents/sequencing/agent.py`: load `LearnerGradeProgress.unlocked_grade` in `_load_topic_ranking_context` (default: the subject's lowest declared grade if no row exists yet), and add the grade-gate clause to `rank_eligible_topics` (depends on T002, T023)
- [ ] T027 [US2] In `backend/src/agents/sequencing/mastery_tool.py`: add the FR-004 unlock check to `apply_mastery_update` (on a topic reaching `mastered` whose grade equals the learner's current `unlocked_grade`, check every topic in that grade is mastered, and if so advance `LearnerGradeProgress.unlocked_grade` to the next declared grade if one exists); add `grade_unlocked: int | None` to `MasteryUpdateResult` (depends on T002, T024)
...
- [ ] T032 [P] [US3] Vitest test for the skip button's replace/remove behavior (depends on T031)
...
- **User Story 2 (Phase 4)**: Depends on Foundational AND User Story 1 (needs a starting grade/`LearnerGradeProgress` row to gate from -- spec.md's own "Why this priority").
...
Task: "Add LearnerGradeProgress model in backend/src/models/learner_grade_progress.py"
