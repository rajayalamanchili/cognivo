...
- [X] T039 (added post-implementation) **Fixed a code-review finding**: `GET /api/quizzes/{quiz_session_id}` (the summary route) had no authorization check at all -- any caller with a `quiz_session_id` could read any learner's quiz summary, and this milestone had just added a mutating side effect (`guardian_viewed_at`) to that same ungated route without noticing. Added `assert_quiz_summary_access` (new function in `assignment.py`, sharing a `_resolve_tier_gated_access` helper with `assert_quiz_session_access` rather than duplicating the tier/guardian/handoff-token logic) -- same gating as `/next-question`, except it deliberately does not require the session to still be `IN_PROGRESS` for a hand-off-token caller, since FR-005c's expiry guarantee is about continuing to *answer*, not reading back a just-finished result. Wired the route to require `claims`/`X-Quiz-Handoff-Token` like `/next-question` already does; threaded a `handoffToken` param through `getQuizSummary()` and `LearnerAssignments.tsx`'s `goToSummary`. **Also found and fixed a related bug this surfaced**: `handleStart`'s ended-early-at-start branch called `goToSummary` synchronously right after `setHandoffToken`, so it was reading React's stale (pre-update) `handoffToken` state instead of the just-received token -- harmless before this fix (the route had no gate to fail), but would have 403'd for any non-co-present-tier learner whose very first assignment attempt ended early. Fixed by passing `result.handoff_token` through explicitly rather than relying on state. New tests: 3 backend integration tests in `test_mediation_tier_gating.py` (co-present requires guardian; hand-off token valid for summary after session completes; token rejected across sessions), 1 frontend regression test in `learner-assignments.test.tsx` for the stale-state fix. `contracts/api.md`'s summary-route section updated (it previously claimed no behavior change was needed here at all). Full backend (`pytest`) + frontend (`Vitest`) regression suites re-run clean after this fix.
- [X] T040 (added post-implementation) **Fixed a second code-review finding**, caught by the automated `claude-code-action@v1` PR review: T039's new access gate still stamped `guardian_viewed_at` for *any* authorized caller, including a hand-off-token-authenticated learner device -- and `LearnerAssignments.tsx` calls the summary route automatically the instant a quiz session ends, on the learner's own device, for every tier except `co_present`. That meant the write almost always landed before the guardian ever looked at anything, defeating FR-006/007/008's opt-in-nudges indicator entirely (it would always read as already-viewed). Fixed by making `_guardian_owns_target` public (`guardian_owns_target`, `assignment.py`) and gating the write in `quiz.py`'s route on it directly, rather than on `assert_quiz_summary_access` having merely passed (which also accepts a hand-off token). New test: `test_learner_viewing_via_handoff_token_does_not_clear_the_indicator` in `test_guardian_viewed_indicator.py` -- views via hand-off token, confirms the indicator still shows for the guardian, then confirms the guardian's own view clears it. `contracts/api.md` updated. Full backend regression suite re-run clean.
- [X] T041 (added post-implementation) **Fixed a third code-review finding**: placement's `submit_placement` route never recorded `read_aloud_used` on its `ANSWER_SUBMITTED` events at all, unlike the practice/quiz answer path -- SC-007 claims 100% reconstructability of read-aloud usage "for a given question," not just quiz/practice ones, and this PR's own summary describes read-aloud as spanning "practice/quiz/placement." Added `read_aloud_used: bool = False` to `PlacementAnswerIn` (`backend/src/api/routes/placement.py`) and included it in the event payload; `placement-flow.tsx` now tracks which questions had their read-aloud control clicked (`readAloudUsed: Record<string, boolean>` state) and sends it per-answer in `submitPlacement`'s batch. New tests: `test_placement_submit_records_read_aloud_used_per_answer` (`test_answer_read_aloud_logging.py`), 1 frontend test in `placement-flow.test.tsx`.
- [X] T042 (added post-implementation) **Fixed a fourth code-review finding**: `pacing.ts`'s `reinforcementEveryN` was computed and unit-tested but never consumed anywhere -- only `recommendedQuestionCount` (the one-time stopping point) was wired in, leaving FR-009's "more frequent positive reinforcement for younger bands" half-implemented. Both `quiz-flow.tsx` and `LearnerAssignments.tsx` now show a brief, non-blocking `reinforcementMessage` ("Nice work! Keep it up! ⭐", `data-testid="reinforcement-message"`) every `reinforcementEveryN` answered questions, computed in the same `advanceAfterAnswer` that already checks the stopping point -- a count that reaches the stopping point suppresses the reinforcement message rather than showing both. `research.md` Decision 6 updated. New tests: 2 in `quiz-flow.test.tsx` (cadence shown/cleared across 4 questions at grade 4; suppressed at grade 1 when the same answer also reaches the stopping point), 1 in `learner-assignments.test.tsx`. Full frontend suite re-run clean (106/106).
