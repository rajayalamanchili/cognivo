...
- [X] T029 [P] Edge-case test: a deliberately tiny `--max-questions-per-topic` budget produces a non-zero `non_converged_count`, and mean/median are computed only over converged learners (not skewed by silently dropping non-convergers from the count) in `backend/tests/unit/evaluation/test_condition_mechanics.py`
- [X] T030 [P] Integration test (SC-006, corrected post-`/speckit-analyze` finding G2): (a) real (`is_demo=False`) `demo_learner_profiles` row count and any pre-existing real `assessment_events` rows are unchanged before vs. after a full harness run; (b) no `eval-harness-*` rows remain in `demo_learner_profiles`, `mastery_states`, or `assessment_events` after the run completes, success or failure -- in `backend/tests/integration/evaluation/test_synthetic_data_cleanup.py`. Verified against a real remote Postgres at small scale (population_size=3, not the default 30 -- see note below); the test file's own default-scale run times out in this sandbox's high-latency DB environment.
- [ ] T031 Run `quickstart.md` validation end to end (all 11 steps) against a local dev environment and record results. **Partially done in this sandbox** -- steps 5, 6, 7, 8, 10 verified live (see notes below); steps 1, 2, 3, 4, 9, 11 need a lower-latency DB environment to complete in reasonable time and were not run to completion here. Re-run fully in a real dev environment before treating this milestone as shipped.
- [ ] T032 Run the full backend `pytest` suite, including Milestone 1's and Milestone 2's existing test directories, and confirm no regressions (SC-007; added post-`/speckit-analyze` finding G1). **Partially done**: an earlier full run (before the FR-004 convergence fix below) showed all 117 Milestone 1/2 + non-DB evaluation tests passing, with only 2 evaluation-specific DB tests failing on connection timeouts caused by the since-fixed non-convergence bug. The convergence fix touched only `backend/src/services/evaluation/`, with no plausible regression path into Milestone 1/2 code. A full fresh run was not completed post-fix due to DB latency in this sandbox -- re-run before shipping.
- [X] T033 [P] Playwright test: the report page renders the headline Sequencing-vs-random result within one screen with no additional navigation required (SC-005; added post-`/speckit-analyze` finding G3), matching this project's existing E2E precedent (`tech-stack.md`'s Testing & evaluation table), in `frontend/tests/e2e/personalization-eval-report.spec.ts`. Run for real against a live local backend+frontend; passed.
