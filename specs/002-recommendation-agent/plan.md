...
| V. Logged and explainable | FR-008: every flag and suggestion becomes its own `AssessmentEvent` row (new `RECOMMENDATION_REPORT_GENERATED` / `WEAK_AREA_FLAGGED` / `NEXT_STEP_SUGGESTED` types) -- unconditional, every request. The tracing half of this principle is conditional on an LLM/ADK invocation occurring (none does here, per research.md §1), so a typical request emits no Langfuse span, matching the existing `GET /mastery-state` precedent; explainability is carried entirely by the audit log for this agent. **Discovered during implementation, not anticipated at plan time** -- see spec.md FR-008 (amended) and tasks.md T021. | PASS |
