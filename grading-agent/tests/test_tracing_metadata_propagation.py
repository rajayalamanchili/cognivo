"""Integration test: `tracing.traced_exchange(question_id=..., learner_id
=...)` actually lands `metadata.question_id`/`metadata.learner_id` on
every span an agent invocation produces.

Mirrors `backend/tests/integration/test_tracing_metadata_propagation.py`
and `tutor-agent/tests/test_tracing_metadata_propagation.py`'s harness
exactly (same `LangfuseSpanProcessor`-attached `TracerProvider`
reasoning: a bare provider would prove nothing about propagation).
Exists because this service's Langfuse trace previously had no link
back to the backend's own `question_id`/`learner_id` at all (found
during the T038 grounding investigation, roadmap.md, and applied here
too since this service has the identical gap) -- `traced_exchange()`
closes that gap the same way `traced_request()` already does on the
backend side.
"""
...
APP_NAME = "cognivo-grading-agent-tracing-metadata-propagation-test"


class _FakeLlm(BaseLlm):
    """Mirrors the backend/tutor-agent tests' fake model -- exercises
...
@pytest.mark.asyncio
async def test_question_and_learner_metadata_propagate_to_every_span(
    in_memory_langfuse_tracing,
):
    session_service = InMemorySessionService()

    with traced_exchange(question_id="question-abc-123", learner_id="learner-xyz"):
    ...
    for span in spans:
        ...
        assert (
            attributes.get("langfuse.trace.metadata.question_id") == "question-abc-123"
        ), f"span {span.name!r} missing propagated question_id metadata: {dict(attributes)}"
        assert (
            attributes.get("langfuse.trace.metadata.learner_id") == "learner-xyz"
        ), f"span {span.name!r} missing propagated learner_id metadata: {dict(attributes)}"


@pytest.mark.asyncio
async def test_no_question_id_means_no_metadata_written(in_memory_langfuse_tracing):
    """No `question_id` (an unauthenticated request never reaches
    ...
    with traced_exchange(question_id=None, learner_id=None):
        await _run_one_agent_invocation(session_service)
    ...
    for span in spans:
        attributes = span.attributes or {}
        assert "langfuse.trace.metadata.question_id" not in attributes
