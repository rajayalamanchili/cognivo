...
def test_resolve_model_llm_provider_wins_over_a_pinned_specific_var(monkeypatch):
    ...
    monkeypatch.setenv("LLM_PROVIDER", "openai")
    monkeypatch.setenv("TUTOR_AGENT_MODEL", "anthropic/claude-sonnet-5")
    assert resolve_model("TUTOR_AGENT_MODEL", "capable").startswith("openai/")


def test_resolve_model_specific_var_still_wins_when_llm_provider_is_unset(monkeypatch):
    monkeypatch.delenv("LLM_PROVIDER", raising=False)
    monkeypatch.setenv("TUTOR_AGENT_MODEL", "openai/gpt-5.6-sol")
    assert resolve_model("TUTOR_AGENT_MODEL", "capable") == "openai/gpt-5.6-sol"


def test_resolve_model_falls_back_to_provider_default_when_neither_is_set(monkeypatch):
    monkeypatch.delenv("LLM_PROVIDER", raising=False)
    monkeypatch.delenv("TUTOR_AGENT_MODEL", raising=False)
    assert resolve_model("TUTOR_AGENT_MODEL", "capable") == "anthropic/claude-sonnet-5"
